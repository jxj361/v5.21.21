...
function renderChatList() {
    ...
    const items = allChats.filter(it => !appData.hiddenChats.includes(it.id)).map(it => {
        ...
        return `<div class="chat-item" data-id="${it.id}" style="position:relative;">
            ...
            <div class="info">
                <div class="name">${escapeHtml(it.name)}${it.members ? ` (${it.members.length})` : ''}</div>
...
function renderContactList() {
    ...
    if (appData.hiddenChats.length > 0) {
        ...
        allContacts.forEach(contact => {
            ...
            const info = document.createElement('div'); info.className = 'info';
            const nameDiv = document.createElement('div'); nameDiv.className = 'name'; nameDiv.textContent = contact.name; nameDiv.style.opacity = '0.5';
            info.appendChild(nameDiv);
...
function exportUserVoices(userId) {
    ...
    const exportData = voices.map(v => ({ name: v.name, data: v.data }));
...
function renderMePage() {
    ...
    resetBtn.onclick = () => {
        showConfirm('重置所有数据？', () => {
            ...
            renderChatList(); renderContactList(); toast('数据已重置');
        });
    };
...
function exportContactChat(userId) {
    const u = getContact(userId); if (!u) return;
    const now = new Date(); const dateStr = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`; const timeStr = `${String(now.getHours()).padStart(2, '0')} ${String(now.getMinutes()).padStart(2, '0')} ${String(now.getSeconds()).padStart(2, '0')}`;
    const blob = new Blob([JSON.stringify({ chat: appData.msg[userId] || [] })], { type: 'application/json' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `jxj_contact_chat_${u.name}_${dateStr}_${timeStr}.json`; a.click(); URL.revokeObjectURL(a.href); toast('联系人聊天已备份');
}
...
// ========== 联系人备份/导出 ==========
function exportContactData(userId) { const u = getContact(userId); if (!u) return; const now = new Date(); const dateStr = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`; const timeStr = `${String(now.getHours()).padStart(2, '0')} ${String(now.getMinutes()).padStart(2, '0')} ${String(now.getSeconds()).padStart(2, '0')}`; const data = { id: u.id, name: u.name, avt: u.avt, bg: u.bg, status: u.status, noReply: u.noReply, noDisturb: u.noDisturb, allowInitiative: u.allowInitiative, soundEnabled: u.soundEnabled, replyTime: u.replyTime, rule: u.rule, cardGroups: u.cardGroups, emojis: appData.userEmo[userId] || [], images: appData.userImages[userId] || [], bgs: appData.userBgs[userId] || [], chat: appData.msg[userId] || [], moments: appData.moments[userId] || [], letters: (appData.letters[userId] || []).filter(l => l.from === 'me'), receivedLetters: (appData.letters[userId] || []).filter(l => l.from !== 'me'), drafts: appData.letterDrafts[userId] || [], momentsBg: appData.momentsBgs[userId] || '' }; const blob = new Blob([JSON.stringify(data)], { type: 'application/json' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `jxj_contact_${u.name}_${dateStr}_${timeStr}.json`; a.click(); URL.revokeObjectURL(a.href); toast('联系人数据已备份'); }

document.getElementById('contactRestoreInput').onchange = function(e) { const file = e.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = function(ev) { try { const result = parseFileContent(ev.target.result, file.name); if (result.parseError) { toast('文件格式错误，无法解析'); return; } if (result.noValidData || (result.cards.length === 0 && result.groups.length === 0)) { toast('未找到有效数据'); return; } showImportModeDialog((mode) => { importContactData(currentChatId, { customReplyGroups: result.groups }, mode); }); } catch { toast('文件损坏，导入失败'); } }; reader.readAsText(file); e.target.value = ''; };
...
function importContactData(userId, data, mode) { const u = getContact(userId); if (!u) return; if (data.customReplyGroups && Array.isArray(data.customReplyGroups)) { const groups = data.customReplyGroups.filter(g => g.name && g.list); if (groups.length > 0) { if (mode === 'overwrite') { u.cardGroups = groups.map(g => ({ name: g.name, list: g.list })); } else { groups.forEach(g => { const existing = u.cardGroups.find(eg => eg.name === g.name); if (existing) existing.list = uniqueArr([...existing.list, ...g.list]); else u.cardGroups.push({ name: g.name, list: g.list }); }); } markDataChanged(); save(); toast(`识别到 milk 格式，导入 ${groups.length} 个分组`); return; } } if (mode === 'overwrite') { if (data.name) u.name = data.name; if (data.avt !== undefined) u.avt = data.avt; if (data.bg !== undefined) u.bg = data.bg; if (data.status) u.status = data.status; u.noReply = !!data.noReply; u.noDisturb = !!data.noDisturb; u.allowInitiative = data.allowInitiative !== undefined ? data.allowInitiative : true; u.soundEnabled = data.soundEnabled !== undefined ? data.soundEnabled : true; if (data.replyTime) u.replyTime = data.replyTime; if (data.rule) u.rule = data.rule; if (data.cardGroups) u.cardGroups = data.cardGroups; appData.userEmo[userId] = data.emojis || []; appData.userImages[userId] = data.images || []; appData.userBgs[userId] = data.bgs || []; appData.msg[userId] = data.chat || []; if (data.moments) appData.moments[userId] = data.moments; if (data.momentsBg !== undefined) appData.momentsBgs[userId] = data.momentsBg; appData.letters[userId] = [...(data.letters || []), ...(data.receivedLetters || [])]; appData.letterDrafts[userId] = data.drafts || []; } else { if (!u.name && data.name) u.name = data.name; if (!u.avt && data.avt) u.avt = data.avt; if (!u.bg && data.bg) u.bg = data.bg; if (!u.status && data.status) u.status = data.status; u.rule = data.rule || u.rule; if (data.cardGroups) { data.cardGroups.forEach(g => { const existing = u.cardGroups.find(eg => eg.name === g.name); if (existing) existing.list = uniqueArr([...existing.list, ...g.list]); else u.cardGroups.push({ name: g.name, list: g.list }); }); } if (data.emojis) appData.userEmo[userId] = uniqueArr([...(appData.userEmo[userId] || []), ...data.emojis]); if (data.images) appData.userImages[userId] = uniqueArr([...(appData.userImages[userId] || []), ...data.images]); if (data.bgs) appData.userBgs[userId] = uniqueArr([...(appData.userBgs[userId] || []), ...data.bgs]); if (data.chat) { if (!appData.msg[userId]) appData.msg[userId] = []; const existingKeys = new Set(appData.msg[userId].map(m => getMsgKey(m))); data.chat.forEach(m => { if (!existingKeys.has(getMsgKey(m))) appData.msg[userId].push(m); }); } if (data.moments) { if (!appData.moments[userId]) appData.moments[userId] = []; const em = new Set(appData.moments[userId].map(m => m.time)); data.moments.forEach(m => { if (!em.has(m.time)) appData.moments[userId].push(m); }); } if (data.momentsBg && !appData.momentsBgs[userId]) appData.momentsBgs[userId] = data.momentsBg; if (data.letters || data.receivedLetters) { if (!appData.letters[userId]) appData.letters[userId] = []; const ec = new Set(appData.letters[userId].map(l => l.cid)); [...(data.letters || []), ...(data.receivedLetters || [])].forEach(l => { if (!ec.has(l.cid)) appData.letters[userId].push(l); }); } if (data.drafts) { if (!appData.letterDrafts[userId]) appData.letterDrafts[userId] = []; const ed = new Set(appData.letterDrafts[userId].map(d => d.id)); data.drafts.forEach(d => { if (!ed.has(d.id)) appData.letterDrafts[userId].push(d); }); } } markDataChanged(); save(); if (currentChatId === userId) openChat(); toast('导入成功'); }
...
// ========== 模拟朋友圈数据 ==========
function simulateUserMoments() { if (Object.keys(appData.moments).length > 0) return; appData.users.forEach(u => { if (!appData.moments[u.id]) appData.moments[u.id] = []; const count = 1 + Math.floor(Math.random() * 3); for (let i = 0; i < count; i++) { const msgs = ['今天天气真好', '有点想你了', '好开心呀', '无聊中...', '哈哈哈']; appData.moments[u.id].push({ name: u.name, avatar: u.avt, status: u.status, text: msgs[Math.floor(Math.random() * msgs.length)], images: [], time: new Date(Date.now() - Math.random() * 86400000).toISOString(), likes: Math.floor(Math.random() * 5), likedBy: [], comments: [] }); } }); markDataChanged(); save(); }
function simulateUserMomentComments() { let changed = false; Object.keys(appData.moments).forEach(uid => { if (uid === 'me') return; const moments = appData.moments[uid]; if (!moments) return; moments.forEach(m => { if (Math.random() < 0.3 && m.comments.length < 3) { const otherUsers = appData.users.filter(u => u.id !== uid); if (otherUsers.length > 0) { const commentUser = otherUsers[Math.floor(Math.random() * otherUsers.length)]; m.comments.push({ name: commentUser.name, userId: commentUser.id, text: '好看！', time: new Date().toISOString() }); changed = true; } } }); }); if (changed) { markDataChanged(); save(); } }
function simulateUserOwnComments() {}
function simulateUserMomentLikes() {}
